...
async function initDbPool() {
    ...
    try {
        ...
        console.log(isConfigured ? ">> [DB] Connected & Tables Found." : ">> [DB] Connected but Tables Missing.");
        return true;
    } catch (e) {
...
// 2. Central API Handler (Robust routing for Hostinger)
app.all(['/admission-api', '/admission-api/'], async (req, res) => {
    // Handle GET status
    if (req.method === 'GET') {
        return res.json({ 
            online: true, 
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            if (action === 'perform_install') {
                ...
                // Update environment logic here would normally go to .env, 
                // but since we can't easily write to .env in all hosting, 
                // we rely on the initDbPool to pick up the new state.
                isConfigured = true;
                await initDbPool();
                return res.json({ status: 'success' });
            }

            if (!isConfigured || !pool) return res.status(503).json({ error: 'Database not initialized. Please run installer.' });
...
if (fs.existsSync(buildPath)) {
    ...
    // Explicit route for installer.html
    ...
    app.get('/', (req, res) => res.send('Backend is running, but build folder is missing. Run npm run build.'));
}
